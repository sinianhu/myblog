...
function ChangeToPointX(x) {
	return parseInt((x - xLeftValue) / (xRightValue - xLeftValue) * FUN_IMG_WIDTH);
}

function cY(y) {
	return  FUN_IMG_HEIGHT - parseInt((y - yLeftValue) / (yRightValue - yLeftValue) * FUN_IMG_HEIGHT);
}
...
//画点
function drawPoint(x,y,flag) {
	
	var lx = ChangeToPointX(x);
	var ly = ChangeToPointY(y);
	
    // 设置绘制颜色
    CONTEXT_2D.fillStyle = "red";
    // 绘制成矩形
    CONTEXT_2D.fillRect(lx - 2, ly - 2, 4, 4);
    
    // 设置字体样式
    CONTEXT_2D.font = "16px bold 宋体";
    // 绘制文字
    //CONTEXT_2D.fillText("(" + x + ", " + y + ")", lx, ly);
	if(flag){
		CONTEXT_2D.fillText("(" + x + ", " + y + ")", lx, ly);
	}
